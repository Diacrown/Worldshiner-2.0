...
// A global admin may explicitly target another office via ?office=CODE;
// everyone else is always pinned to their own office_id from the token —
// this is the direct fix for the old system's `office: 'DM-USA'` literal
// hardcoded per file, which is what let a copy-pasted file silently mistag
// another office's jobs.
export async function resolveWriteOfficeId(req) {
  ...
  // Everyone else is always pinned to their own office_id from the token —
  // the direct fix for the old system's `office: 'DM-USA'` literal hardcoded
  // per file, which is what let a copy-pasted file silently mistag another
  // office's jobs.
