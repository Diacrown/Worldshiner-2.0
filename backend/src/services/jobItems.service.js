...
// Mints the next sequential design number. Claiming is separate from
// creating an entry: a matching set claims ONE number, then multiple pieces
// (each with their own suffix) reuse that same base — so the number is
// handed to the caller first, and addDesignEntry() below just records it.
export async function claimDesignNumber(user, jobId) {
  const job = await getJobById(user, jobId);
  if (!job) return null;
  return withTransaction(async (client) => {
    const { rows } = await client.query(
      'UPDATE design_number_counter SET next_value = next_value + 1 WHERE id = 1 RETURNING next_value - 1 AS claimed'
    );
    return { baseNumber: formatDesignNo(rows[0].claimed) };
  });
}

export async function addDesignEntry(user, jobId, { kind, baseNumber, suffix, description, poNumber, sortOrder }) {
  const job = await getJobById(user, jobId);
  if (!job) return null;
  ...
  return withTransaction(async (client) => {
    const { rows } = await client.query(
      `INSERT INTO job_design_entries (job_id, kind, base_number, description, po_number, sort_order)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [jobId, kind, finalBaseNumber, description || null, poNumber || null, sortOrder || 0]
    );
